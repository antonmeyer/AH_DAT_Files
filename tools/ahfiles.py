...
def updateCheckSum (scenearray):
    ...
    a2 = binascii.unhexlify('%x' % checksum)
    b2 = array('B',a2)
    b2.reverse()
...
def writeSceneFile (scenearray, numstr):
    ...
    filename = args.outpath +"/" +"SCENE" + numstr +".DAT"
    with open(filename, 'w') as ofile:
        scenearray.tofile(ofile)

    return
def setMuteList(scenearray, mutelist):
    "set all 24 mute as in the mutelist of 0s and 1s"
...
def processMuteFile():
    ...
    with open(args.mutefile, 'rb') as mutefile:
        ...
        for row in mfreader:
            ...
            rownum = format(int(row[0]), '03d') #first line is scenenumber
            setSceneName(scenearray,"Scene"+rownum)
            writeSceneFile(scenearray,rownum)
    return
...
CLI.add_argument(
 ...
 nargs=1,  # 1 inpute file
 default="../SHOW0011/SCENE100.DAT",  # default if nothing is provided
)
CLI.add_argument(
 "--outpath",  # name on the CLI - drop the `--` for positional/required parameters
 nargs=1,  # 1 inpute file
 default="../SHOW0111",  # default if nothing is provided
)
CLI.add_argument(
 "--scenename",  # name on the CLI - drop the `--` for positional/required parameters
 nargs=1,  # 1 inpute file
 default="",  # default if nothing is provided
)
CLI.add_argument(
 "--showname",  # name on the CLI - drop the `--` for positional/required parameters
 nargs=1,  # 1 inpute file
 default="",  # default if nothing is provided
)
CLI.add_argument(
 "--wcrc",  # name on the CLI - drop the `--` for positional/required parameters
 type=bool,
 default=False,  # default if nothing is provided
)
CLI.add_argument(
 "--unmutelist",  # name on the CLI - drop the `--` for positional/required parameters
 type=int,
 nargs='+',
 default=[],  # default if nothing is provided
)
...
"--mutefile",
nargs=1,
default="./mutefile.csv"
)

# parse the command line
args = CLI.parse_args()
# access CLI options
print("input: %r" % args.input)
print("output: %r" % args.output)
print("wcrc: %r" % args.wcrc)
print("unmutelist: %r" % args.unmutelist)
