...
maxColumn = 32 # 24 or 32 Faders

def calcCheckSum (scenearray):
    "retuns the CheckSum of a Scene filedata array"
    lut = [0x00000000, 0x77073096, 0xEE0E612C, 0x990951BA, 0x076DC419, 0x706AF48F, 0xE963A535, 0x9E6495A3,
...
def setSceneName(scenearray, scenename):
    ...
    #a = array('B', scenename)
    a = bytearray (str.encode (scenename))
    alen = len(a)
    if alen > 14:
...
def writeSceneFile (scenearray, numstr):
    ...
    ofile.write(scenearray)
    ofile.close()

    return

def getAllValuesAt(parapos):
    "prints for all scenes values at parameter position for  channels"
    filefilter = os.path.join(args.showdir, 'SCENE???.DAT')
    datfiles = glob.glob(filefilter)
    datfiles.sort()
...
def processMuteFile():
    ...
    with open(args.mutefile, 'rtU') as mutefile:
...
#old outdated
def setMuteList(scenearray, row):
    ...
    for i in range(1, len(row)): #first col is scenenumber, we want values
        if ((row[i] == "1") or (row[i] =="0")):
            scenearray[0xb8 + (i-1)*0xC0] = int(row[i]) #first col is scene number
    return
...
CLI.add_argument(
 "--addbefore", 
 help= "dublicate the given scene (number) before, shift the others file numbers up",
)
CLI.add_argument(
"--getSceneNames",
